...
class TextRecognizer(BaseOCRV20):
    def __init__(self, args, **kwargs):
        ...
        self.weights_path = 'C:\\Users\\kiko\\Desktop\\paddleocr_torch\\PaddleOCR2Pytorch\\ch_ptocr_mobile_v2.0_rec_infer.pth'
...
def main(args):
    image_file_list = get_image_file_list(args.image_dir) # 读取文件列表
    text_recognizer = TextRecognizer(args) # 文字识别继承
    valid_image_file_list = []
    img_list = []
    for image_file in image_file_list: #遍历每张图片
        ...
        img_list.append(img)
        # print(type(img_list)) # list文件
        # print(len(img_list)) # 1个元素
    try:
...
def main(args):  # 输入h,w,c矩阵

    img_list = []
    text_recognizer = TextRecognizer()
    img_list.append(input_mat)
    try:
        rec_res, predict_time = text_recognizer(img_list)
